...
| 2026-04-20 | Remaining 5 sections pinned | migrations 01067-01071 via `npm run migrate` + `scripts/qa/verify-pins.ts` × 5 | 5,629 / 8,612 pinned (65.4% bank-wide). All 6 sections verified: pin counts match deduped classifier exactly. |
| 2026-04-20 | SKILL_TO_BLOOM bug fix + Wave B re-run | `scripts/qa/audit-task-coverage.ts` after `SKILL_TO_BLOOM: {"Remembering and Understanding": 1}` fix | Wave B "AICPA has zero L1, bank has ~2,500 excess L1" finding REVERSED. AICPA 2026 actually has 25-59% L1 per section; bank is close to or slightly under target. |
...
| ~~4~~ | ~~REG Individual Taxation proliferation — 5 topic strings~~ | ~~Phase 3 re-tagging~~ | ~~Decide: merge or split~~ | **SCOPE-CLOSED 2026-04-20.** REG was accepted at 42.4% match 2026-04-19 (queue of "deferred content-level work" covers this). Merging or splitting 319 REG Individual Tax questions is not blocking any Direction W phase now; it would be part of a REG content-level rewrite if/when that is undertaken. Not tracked as active cleanup. |
| ~~5~~ | ~~4 slash-named mixed-content topics~~ | ~~Phase 3 re-tagging~~ | ~~Resolve alongside #4~~ | **SCOPE-CLOSED 2026-04-20.** 2 of the 4 (Property Transactions: Gains/Losses, Review/Compilation Engagements) are legitimate AICPA standard naming — no action. The other 2 (Credits/AMT, Filing/Credits) are REG Individual Tax mixed-content artifacts — scope-covered under #4. |
...
| ~~9~~ | ~~Migration ledger delta: 5 applied migrations with no disk file~~ | ~~Clean-end-state rule "ledger in sync"~~ | ~~Investigate~~ | **CLOSED 2026-04-20** via migration 01073. The 5 phantoms were: 3 unfilled scaffold migrations deleted in commit `d6da50c` (00252/00279/00281) + 2 renumbered migrations (00571→00583, 00578→00584). Migration 01073 deletes the 5 phantom rows from `applied_migrations`. Ledger now reports 1054 on-disk = 1054 applied. |
